...
function createOrgs() {
  ...
  infoln "Generating CCP files for Org1 and Org2, Org3"
...
# file for creating the network is "docker-compose-fabric-net.yaml" in the ``docker``
...
function explorerUp() {
  COMPOSE_FILES="-f compose/${COMPOSE_FILE_EXPLORER} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_EXPLORER}"

  DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d 2>&1

  $CONTAINER_CLI ps -a
  if [ $? -ne 0 ]; then
    fatalln "Unable to start explorer"
  fi
}
...
function networkDown() {
  ...
  COMPOSE_EXPLORER_FILES="-f compose/${COMPOSE_FILE_EXPLORER} -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_EXPLORER}"
  COMPOSE_FILES="${COMPOSE_BASE_FILES} ${COMPOSE_COUCH_FILES} ${COMPOSE_CA_FILES} ${COMPOSE_EXPLORER_FILES}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_raft0.example.com docker_raft1.example.com docker_raft2.example.com docker_peer0.org1.example.com docker_peer1.org1.example.com docker_peer0.org2.example.com docker_peer1.org2.example.com docker_peer0.org3.example.com docker_peer1.org3.example.com
...
COMPOSE_FILE_BASE=compose-fabric-net.yaml
# docker-compose.yaml file if you are using couchdb
COMPOSE_FILE_COUCH=compose-couch.yaml
# certificate authorities compose file
COMPOSE_FILE_CA=compose-ca.yaml
# explorer docker-compose file
COMPOSE_FILE_EXPLORER=compose-explorer.yaml
...
if [ "$MODE" == "up" ]; then
  ...
  createChannel
  explorerUp # start explorer after channel creation
elif [ "$MODE" == "down" ]; then

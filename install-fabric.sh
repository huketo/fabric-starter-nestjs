...
# jq is required to parse the github api response
...
_arg_fabric_version=$(curl -s https://api.github.com/repos/hyperledger/fabric/releases/latest | jq -r '.tag_name' | sed 's/^v//')
_arg_ca_version=$(curl -s https://api.github.com/repos/hyperledger/fabric-ca/releases/latest | jq -r '.tag_name' | sed 's/^v//')
...
print_help() {
    ...
    printf '\t%s\n' "<comp> Component to install, one or more of  docker | binary | podman  First letter of component also accepted; If none specified docker | binary | samples is assumed"
